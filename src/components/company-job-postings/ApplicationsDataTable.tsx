...
import {
  COMPANY_APPLICATION_STATUS_VALUES,
  normalizeCompanyApplicationStatus,
...
export function ApplicationsDataTable({
  ...
  loading,
  showJobColumns,
  emptyMessage = "No applications for this listing yet.",
  isRowUpdating,
  onStatusChange,
  className,
  ...
  const pendingFrom = pendingRow
    ? statusLabel(normalizeCompanyApplicationStatus(pendingRow.status))
    : ""
  const pendingTo = pendingNext ? statusLabel(pendingNext) : ""
  const confirmDisabled =
    pendingRow != null && isRowUpdating(pendingRow)

  return (
    <>
      <div
        className={cn(
          "relative w-full overflow-x-auto rounded-xl border border-border/70 bg-white",
          className
        )}
      >
      <table className="w-full min-w-[960px] border-collapse text-left text-sm">
        <thead>
          <tr className="border-b border-border/60 bg-muted/40">
            {showJobColumns ? (
              <>
                <th className="whitespace-nowrap px-3 py-3 font-medium text-muted-foreground sm:px-4">
                  Job ID
                </th>
                <th className="min-w-[8rem] whitespace-nowrap px-3 py-3 font-medium text-muted-foreground sm:px-4">
                  Job
                </th>
              </>
            ) : null}
            ...
            <th className="min-w-[8.5rem] whitespace-nowrap px-3 py-3 font-medium text-muted-foreground sm:px-4">
              Status
            </th>
            <th className="min-w-[10rem] whitespace-nowrap px-3 py-3 font-medium text-muted-foreground sm:px-4">
              Submitted
            </th>
            {!showJobColumns ? (
              <th className="min-w-[8rem] whitespace-nowrap px-3 py-3 font-medium text-muted-foreground sm:px-4">
            ...
            <th className="min-w-[6rem] whitespace-nowrap px-3 py-3 font-medium text-muted-foreground sm:px-4">
              Name
            </th>
            <th className="min-w-[10rem] whitespace-nowrap px-3 py-3 font-medium text-muted-foreground sm:px-4">
              Email
            </th>
            <th className="min-w-[7rem] whitespace-nowrap px-3 py-3 font-medium text-muted-foreground sm:px-4">
              Phone
            </th>
            <th className="min-w-[7rem] whitespace-nowrap px-3 py-3 font-medium text-muted-foreground sm:px-4">
              LinkedIn
        ...
        <tbody>
          {rows.map((row) => {
            ...
            const linkedin =
              row.linkedin != null && String(row.linkedin).trim() !== ""
            ...
            return (
              <tr
                key={
                  showJobColumns && row.jobId
                    ? `${row.jobId}-${row.id}`
                    : String(row.id)
                }
                className="border-b border-border/40 last:border-b-0 even:bg-muted/10"
              >
                {showJobColumns ? (
                  <>
                    <td className="px-3 py-2.5 font-mono text-xs text-foreground sm:px-4">
                      {row.jobId ?? "—"}
                    </td>
                    <td className="wrap-break-word px-3 py-2.5 text-foreground sm:px-4">
                      {row.jobTitle?.trim() || "—"}
                    </td>
                  </>
                ) : null}
                ...
                <td className="wrap-break-word px-3 py-2.5 text-muted-foreground sm:px-4">
                ...
                <td className="wrap-break-word px-3 py-2.5 font-medium text-foreground sm:px-4">
                  {row.name?.trim() || "—"}
                </td>
                ...
                <td className="wrap-break-word px-3 py-2.5 font-mono text-xs text-foreground sm:px-4">
                  {cv ?? "—"}
                </td>
              </tr>
            )
              ...
              {pendingRow ? (
                <>
                  <span className="font-medium text-foreground">
                    {pendingRow.name?.trim() || "Applicant"}
                  </span>
