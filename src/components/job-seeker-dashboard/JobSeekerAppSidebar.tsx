...
function navItemActive(pathname: string, to: string) {
	...
	if (to === "/seeker/dashboard") return false
...
export function JobSeekerAppSidebar() {
			...
			<SidebarContent className="bg-white md:mt-15 ">
				<SidebarGroup>
					...
					<SidebarGroupContent className="gap-5">
						<SidebarMenu>
							{seekerNav.map(({ to, label, icon: Icon }) => (
								<SidebarMenuItem key={to} className="mb-3">
									<SidebarMenuButton
										asChild
										isActive={navItemActive(pathname, to)}
										tooltip={label}
										className="py-6 px-3 duration-300 ease-out transition-colors motion-reduce:transition-none"
									>
										<NavLink
											to={to}
								...
								>
									<Avatar className="size-8 rounded-lg">
										<AvatarFallback className="rounded-lg bg-sidebar-primary text-xs font-semibold text-sidebar-primary-foreground">
											{accountInitial(displayName, displayEmail)}
										</AvatarFallback>
									</Avatar>
							...
							>
								...
								<DropdownMenuGroup>
									<DropdownMenuItem asChild className="rounded-lg">
										<Link to="/seeker/profile" onClick={closeMobile}>
