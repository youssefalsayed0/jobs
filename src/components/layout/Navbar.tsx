		...
		<header className="sticky top-0 z-50 border-b border-slate-200/80 bg-white/95 px-4 py-3.5 backdrop-blur">
			<div className="mx-auto flex container items-center justify-between gap-4">
				<Link to="/" className="flex items-center gap-2">
					...
					<span className="text-2xl font-bold tracking-tight text-slate-900">Opportix</span>
				</Link>

				<nav className="hidden items-center gap-7 text-sm font-medium md:flex">
					{navLinks.map((item) => (
						<NavLink key={item.to} to={item.to} className={({ isActive }) => cn("text-slate-600 transition hover:text-slate-900", isActive && "font-semibold text-slate-900")}>
							{item.label}
						</NavLink>
					))}
				</nav>
				...
				<div className="flex items-center gap-3">
					{isLoggedIn ? (
						<>
							...
							<Button onClick={onLogout} variant="outline" size="sm" className="rounded-lg">
								Log out
							</Button>
							{userAvatar ? (
								<img src={userAvatar} alt="User avatar" className="size-10 rounded-full object-cover ring-2 ring-slate-200" />
							) : (
								<div className="flex size-10 items-center justify-center rounded-full bg-blue-600 text-sm font-semibold text-white">{userName?.charAt(0)?.toUpperCase() || "U"}</div>
							)}
						</>
					) : (
