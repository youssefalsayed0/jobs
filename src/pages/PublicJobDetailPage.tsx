  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col bg-linear-to-b from-muted/40 via-background to-muted/20">
      <header className="relative border-b border-border/60 bg-card shadow-sm">
        <div
          className="pointer-events-none absolute -right-24 -top-32 size-72 rounded-full bg-primary/9 blur-3xl sm:size-96"
          aria-hidden
        />
        ...
        <div className="container relative mx-auto w-full px-4 py-8 sm:px-8 sm:py-10 lg:py-12">
          <Button
            variant="ghost"
            size="sm"
            className="mb-6 -ms-1 gap-2 rounded-full text-muted-foreground hover:bg-muted/80 hover:text-foreground"
            asChild
          >
            <Link to="/jobs">
          ...
          <div className="flex flex-col gap-8 lg:flex-row lg:items-start lg:justify-between lg:gap-12">
            <div className="min-w-0 flex-1 space-y-5">
              <div className="flex flex-wrap items-center gap-2">
                {workType ? (
                  <Badge
                    variant="secondary"
                    className="rounded-lg px-2.5 py-0.5 text-xs font-medium capitalize"
                  >
                    {workType}
                  </Badge>
                ) : null}
                {location ? (
                  <Badge
                    variant="outline"
                    className="gap-1.5 rounded-lg border-border/80 px-2.5 py-0.5 text-xs font-normal"
                  >
            ...
            <div className="flex w-full shrink-0 flex-col gap-4 sm:flex-row sm:items-center lg:w-72 lg:flex-col lg:items-stretch">
              <Button
                type="button"
                size="lg"
                className="h-11 w-full rounded-xl font-semibold shadow-md sm:h-12"
                onClick={openApply}
              >
              ...
              {user && !isSeeker ? (
                <Alert className="rounded-2xl border-amber-500/25 bg-amber-500/6">
                  ...
                  <AlertDescription className="text-amber-950/80 dark:text-amber-50/85">
                    Job applications are for job seeker accounts. Post and
                    manage roles from your company dashboard.
                  </AlertDescription>
                </Alert>
              ) : (
                <p className="text-center text-xs leading-relaxed text-muted-foreground lg:text-left">
                  {user && isSeeker
                    ? "You will attach your CV and contact details in the next step."
      ...
      <div className="container mx-auto w-full flex-1 space-y-6 px-4 py-8 sm:space-y-8 sm:px-8 sm:py-10 lg:py-12">
            ...
            <CardHeader className="border-b border-border/50 bg-muted/15 px-6 py-6 sm:px-8 sm:py-7">
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 space-y-1">
                  ...
                  <CardDescription>
                    What you will do and how the team works.
                  </CardDescription>
                </div>
              </div>
            </CardHeader>
            ...
            <CardHeader className="border-b border-border/50 bg-muted/15 px-6 py-6 sm:px-8 sm:py-7">
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 space-y-1">
                  ...
                  <CardDescription>
                    Skills and experience the team is looking for.
                  </CardDescription>
                </div>
              </div>
            </CardHeader>
            ...
            <CardHeader className="border-b border-border/50 bg-muted/15 px-6 py-6 sm:px-8 sm:py-7">
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 space-y-1">
                  ...
                  <CardDescription>
                    Education and credentials that strengthen your application.
                  </CardDescription>
                </div>
              </div>
            </CardHeader>
        ...
        {!job.description && !job.requirements && !job.qualification ? (
          <Card className="rounded-3xl border-dashed border-border/80 bg-muted/15 py-12 text-center shadow-none">
            <CardContent className="text-sm text-muted-foreground">
              No extended description has been provided for this listing yet.
            </CardContent>
          </Card>
        ) : null}
        ...
        <div className="flex flex-col items-center justify-between gap-4 rounded-2xl border border-border/60 bg-muted/20 px-5 py-6 sm:flex-row sm:px-8">
          <p className="text-center text-sm text-muted-foreground sm:text-left">
            Ready to move forward? Your application includes your CV and
            contact details.
          </p>
          <Button
            type="button"
            ...
            onClick={openApply}
          >
            {user && isSeeker ? "Apply now" : "Sign in to apply"}
          </Button>
        </div>
      </div>
      ...
      <Dialog open={applyOpen} onOpenChange={setApplyOpen}>
        <DialogContent className="flex max-h-[min(90vh,720px)] max-w-lg flex-col gap-0 overflow-hidden p-0 sm:max-w-lg">
          <DialogHeader className="shrink-0 space-y-2 border-b border-border/60 bg-muted/10 px-5 py-5 sm:px-6 sm:py-6">
            ...
            <DialogDescription>
              Add your details and CV. You can review everything before
              submitting.
            </DialogDescription>
          </DialogHeader>
          <form
                  ...
                  render={({ field, fieldState }) => (
                    <Field data-invalid={fieldState.invalid}>
                      ...
                      <FieldContent>
                        ...
                        />
                        <FieldDescription>
                          Optional — helps recruiters verify your background.
                        </FieldDescription>
                        {fieldState.invalid ? (
                          <FieldError errors={[fieldState.error]} />
                        ) : null}
