...
function TimelineItem({
  ...
  const ring =
    tone === "amber"
      ? "text-amber-600/90 ring-amber-200/80 bg-amber-50/90"
      : "text-primary ring-primary/15 bg-primary/8"

  return (
    <div className="relative pl-8 pb-4 last:pb-0">
      <span
        className={cn(
          "absolute left-0 top-1 flex size-6 items-center justify-center rounded-full ring-1",
          ring
        )}>
      ...
      <div className="rounded-2xl border border-border/60 bg-card/90 p-4 shadow-sm">
        ...
        {meta?.trim() ? (
          <p className="mt-2 inline-flex rounded-lg bg-muted/50 px-2 py-0.5 font-mono text-xs text-muted-foreground">
            {meta}
          </p>
        ) : null}
        {body?.trim() ? (
    ...
    <div className="mx-auto w-full max-w-full space-y-8 pb-10">
      <section className="rounded-3xl border border-border/80 bg-linear-to-br from-card via-card to-primary/[0.07] px-7 py-8 shadow-sm sm:px-10 sm:py-10">
        <Button
          asChild
          variant="ghost"
          className="mb-6 -ml-2 h-auto justify-start gap-2 rounded-xl px-2 py-1.5 text-muted-foreground hover:bg-background/60 hover:text-foreground">
        ...
        <p className="mt-4 max-w-2xl text-base leading-relaxed text-muted-foreground">
          Review this candidate&apos;s details and history. Use the contact actions to reach out
          or open their CV.
        </p>
      </section>
            ...
            <CardHeader className="space-y-4 border-b border-border/60 bg-muted/15 pb-6">
              <div className="relative mx-auto w-fit">
                ...
                <Avatar className="relative size-28 rounded-[1.75rem] border-2 border-background shadow-md ring-2 ring-border/60 sm:size-32">
                  ...
                  <AvatarFallback className="rounded-[1.65rem] bg-muted text-2xl font-semibold text-muted-foreground">
                    {initialsFromUser(displayName)}
                  </AvatarFallback>
                </Avatar>
              </div>
          ...
          <Card className="rounded-3xl border-border/80 bg-card/90 shadow-sm ring-1 ring-black/3">
            ...
            <CardContent className="space-y-3">
              <ContactRow
                ...
                label="Email"
                value={user.email}
                href={user.email?.trim() ? `mailto:${user.email.trim()}` : undefined}
              />
              <ContactRow
                icon={PhoneIcon}
                label="Phone"
                value={user.phone}
                href={user.phone?.trim() ? `tel:${user.phone.replace(/\s/g, "")}` : undefined}
              />
              <ContactRow icon={UserRoundIcon} label="Gender" value={user.gender} />
              <ContactRow
                icon={MapPinIcon}
                label="Location"
                value={locationLine || null}
              />
            </CardContent>
          </Card>
        </div>
              ...
              <CardHeader className="flex flex-col gap-4 space-y-0 border-b border-border/60 bg-muted/10 sm:flex-row sm:items-end sm:justify-between">
                <div>
                  ...
                  <CardDescription className="mt-1">
                    Education, work history, and certificates
                  </CardDescription>
                </div>
                    ...
                    <CardContent className="space-y-10 p-6 sm:p-8">
                      <section>
                        ...
                        {experiences.length > 0 ? (
                          <div className="relative space-y-2 border-l border-border/70 pl-1">
                            {experiences.map((x) => (
                              <TimelineItem
                                ...
                                icon={BriefcaseIcon}
                                title={
                                  [x.title?.trim(), x.company_name?.trim()]
                                    .filter(Boolean)
                                    .join(" · ") || "Role"
                                }
                          ...
                          <p className="rounded-2xl border border-dashed border-border/70 bg-muted/10 px-4 py-6 text-sm text-muted-foreground">
                          ...
                          <div className="flex gap-3 min-w-0">
                            ...
                            <div className="min-w-0">
                              ...
                              <p className="text-sm text-muted-foreground">
                                {[c.issuer, c.issued_at].filter(Boolean).join(" · ") || "—"}
                              </p>
                            </div>
                          </div>
    ...
    return (
      <div className="mx-auto max-w-lg space-y-6 rounded-3xl border border-border/80 bg-card/80 p-8 shadow-sm">
        ...
        <div className="space-y-2">
          ...
          <p className="text-sm text-muted-foreground">
            This URL does not include a valid applicant id.
          </p>
        </div>
        ...
        <div className="mx-auto max-w-lg space-y-6 rounded-3xl border border-border/80 bg-card/80 p-8 shadow-sm">
          ...
          <div className="space-y-2">
            ...
            <p className="text-sm leading-relaxed text-muted-foreground">
              This applicant could not be loaded. They may have been removed or you may not have
              access.
            </p>
          </div>
          <Button asChild variant="outline" className="rounded-xl">
